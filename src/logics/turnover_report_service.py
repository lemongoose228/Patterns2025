...
class TurnoverReportService:
    ...
    def __init__(self, start_service):
        ...
        LoggerService.log_info("Инициализирован TurnoverReportService", "turnover_report_service")

    def generate_turnover_report(self, start_date: datetime, end_date: datetime, storage: StorageModel = None,
                                 filters: list[FilterDto] = None):
        """
        ...
        # Логирование начала формирования отчета
        storage_name = storage.name if storage else "все склады"
        LoggerService.log_report_generation("ОСВ", start_date, end_date)
        LoggerService.log_info(f"Формирование ОСВ с {start_date} по {end_date}, склад: {storage_name}",
                               "turnover_report_service")

        if start_date > end_date:
            LoggerService.log_error(f"Дата начала ({start_date}) позже даты окончания ({end_date})",
                                    "turnover_report_service")
        ...
        if filters:
            ...
            LoggerService.log_debug(f"Применены пользовательские фильтры: {len(filters)} шт.",
                                    "turnover_report_service")
        else:
            filtered_transactions = filtered_transactions

        LoggerService.log_debug(f"Для отчета отобрано {len(filtered_transactions)} транзакций",
                                "turnover_report_service")
        ...
        LoggerService.log_info(f"ОСВ сформирован: {len(report_data)} позиций", "turnover_report_service")
        return report_data

    def _apply_base_filters(self, prototype: Prototype, start_date: datetime, end_date: datetime,
    ...
    def _build_turnover_report_from_transactions(self, transactions: list, start_date: datetime):
        ...
        LoggerService.log_debug(f"Группировка по {len(nomenclature_data)} номенклатурам", "turnover_report_service")
    ...
    def generate_turnover_report_old(self, start_date: datetime, end_date: datetime, storage: StorageModel = None):
        ...
        """
        LoggerService.log_info(f"Использована старая версия формирования ОСВ с {start_date} по {end_date}",
                               "turnover_report_service")
        ...
        LoggerService.log_info(f"ОСВ (старая версия) сформирован: {len(report_data)} позиций",
                               "turnover_report_service")
        return report_data
    ...
    def _calculate_turnover_for_nomenclature(self, nomenclature, transactions, start_date, end_date, storage,
                                             transaction_type):
